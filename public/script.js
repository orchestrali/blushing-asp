  ...
  socket.on("stand", (n) => {
    if (door) {
      for (let i = 1; i <= numbells; i++) {
        ...
        document.getElementById("rope"+i).setAttributeNS(null, "viewBox", "0 0 "+width+" 500");
  ...
  socket.on("ring", obj => {
    if (door && !phone) {
      ...
      if (bell.stroke === obj.stroke) {
        let y = obj.stroke === 1 ? "173.7 " : "0 ";
        let mbell = mbells.find(b => b.num === obj.bell);
        if (mbell) {
          mbell.ringing = false;
        }
        document.getElementById("rope"+bell.num).setAttributeNS(null, "viewBox", "0 "+y+width+" 500");
        ring(bell.num);
...
`
  ...
  function updateentrant(o, isnew) {
    if (isnew) {
      ...
      $("#entrants").append('<li id="'+o.name.replace(/ /i, "")+'"><span>'+o.name+ '</span>' + c+'<div class="assign'+ d+ '><span class="summary">no bells</span>' + selectOpts(name, o.bells) +'</div></li>');
      
    } else {
      let li = $("li#"+o.name.replace(/ /i, ""));
      ...
      if (o.exit) {
        ...
        for (let i = 1; i <= numbells; i++) {
          $("input#"+o.name.replace(/ /i, "")+"-"+i).prop("checked", o.bells.includes(i));
        }
  ...
  function assign(me) {
    ...
    if (me && me.bells) {
      ...
      me.bells.forEach(b => {
        ...
        sallies.push(document.getElementById("sally"+b));
        tails.push(document.getElementById("tail"+b));
